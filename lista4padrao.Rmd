...
Considerei também retirar os países que estão sem as labels, visto que assim o gráfico fica menos poluído e aumenta a velocidade de interpretação do mesmo.
...
tempa <- data.frame(long = temps$long, alpha = normalize(abs(temps$temp)))
#View(troops$long[troops$direction == "R"])
gettheclosest <- function(x, y){
  alpha <- c()
  for (i in x){
    closest <- abs(i - y)
    index_in_closest <- match(min(closest), closest)
    alpha <- c(alpha, index_in_closest)
  }
  alpha
}

closestlongs <- gettheclosest(troops$long, temps$long)
alpha <- c()

for (i in 1:length(troops$long)) {
  if (troops[i, ]$direction == "A"){
    alpha <- c(alpha, 1)
  } else {
    alpha <- c(alpha, tempa[closestlongs[i], 2])
  }
}

troops <- cbind(troops, alpha)
#View(troops)
```
...
march.1812.plot.simple <- ggplot() +
  ...
  scale_colour_manual(values = c("#AD9236", "#4059AD")) +
...
animate(p, width = 900, height = 900, nframes = 100, fps = 8, end_pause = 32,renderer=gifski_renderer("problem3animation.gif"))
```

Modificação:

```{r}
data <- mydf_filter[mydf_filter$year %% 5 == 0,]
ggplot(data, aes(x = year, y = life, size = pop, color = continent), alpha = fert) +
  ylim(30,100) +
  geom_point() +
  scale_color_brewer(type = 'div', palette = 'Spectral') + 
  labs(x="Year", y = "Life expectancy at birth (years)", caption = "(Based on data from Hans Rosling - gapminder.com)", color = 'Continent',size = "Population (millions)", alpha = "Fertility Rate")
```
...
taxa <-  runif(diasadicionados, min = 1.02, max = 1.034)
...
# Aplicação dos estilos citados acima, adição do título, movimentação da legenda para a parte inferior.
atirei1 <- atirei1 %>% layout(xaxis = list(title = ""),yaxis = style.atirei1y1, yaxis2 = style.atirei1y2, title = paste("Obitos COVID-19 no Brasil Previsão até 06/2020 com taxa constante de ", round(taxa, digits = 3)),legend = list(orientation = "h"), margin = list(r = 50))
# Exibição do gráfico
atirei1
```
